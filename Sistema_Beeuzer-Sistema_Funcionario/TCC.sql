...
delimiter $$
create procedure spSelectProd(vCodBarras bigint)
begin
	select * from vwProduto where CodigoBarras = vCodBarras;
end $$

call spSelectProd(12345678910115);

delimiter $$
create procedure spUpdateProd(vCodigoBarras bigint, vNomeProd varchar(200), vCor varchar(55), vTamanho char(2), vValorUnitario decimal(8, 2), vQtd int)
begin
	update tbl_produto set NomeProd = vNomeProd, Cor = (select IdCor from tbl_cor where Cor = vCor), Tamanho = (select IdTamanho from tbl_tamanho where Tamanho = vTamanho), ValorUnitario = vValorUnitario, Qtd = vQTd where CodigoBarras = vCodigoBarras;
end $$

call spUpdateProd(12345678910115, "Caixa calça", "Colorida", "PP", 69.89, 10);

use db_tcc;
create view vwProduto as
select 
	tbl_produto.CodigoBarras,
    tbl_produto.NomeProd,
    tbl_cor.Cor,
    tbl_tamanho.Tamanho,
    tbl_produto.ValorUnitario,
    tbl_produto.Qtd
from tbl_produto 
inner join tbl_Cor on (tbl_produto.Cor = tbl_cor.IdCor)
inner join tbl_Tamanho on (tbl_produto.Tamanho = tbl_tamanho.IdTamanho);

select * from vwProduto order by CodigoBarras;

delimiter $$
create procedure spInsertEnde(vCep int, vLogradouro varchar(200), vNomeBairro varchar(50), vNomeCidade varchar(50))
begin
	if not exists(select Cep from tbl_enderecoEntrega where vCep = Cep) then
	...
	insert into tbl_enderecoEntrega (Cep, Logradouro, idBairro, IdCidade)
...
select 
	tbl_enderecoEntrega.cep,
    tbl_enderecoEntrega.logradouro,
    tbl_Bairro.NomeBairro,
    tbl_Cidade.NomeCidade
from tbl_enderecoEntrega inner join tbl_Cidade
    on (tbl_enderecoEntrega.idcidade = tbl_cidade.idcidade)
    inner join tbl_Bairro
    on (tbl_enderecoEntrega.Idbairro = tbl_Bairro.IdBairro);
...
delimiter $$
create procedure spInsertCarrinho(vIdCli bigint, vQtd bigint, vCodBarras bigint)
begin
	set @TotalProd = (select ValorUnitario from tbl_produto where CodigoBarras = vCodBarras) * vQtd;
	
    insert into tbl_carrinho (IdCar, TotalProd, Qtd, IdCli, CodigoBarras) values (default, @TotalProd, vQtd, vIdCli, vCodBarras);
end$$ 

delimiter $$
create procedure spSelectCarrinho(vIdCli bigint)
begin
	select
    tbl_carrinho.IdCar,
    tbl_produto.NomeProd,
    ...
    tbl_carrinho.Qtd,
    tbl_produto.ValorUnitario,
    tbl_carrinho.TotalProd
    from tbl_carrinho
    inner join tbl_produto on (tbl_carrinho.CodigoBarras = tbl_produto.CodigoBarras)
    inner join tbl_Cor on (tbl_produto.Cor = tbl_cor.IdCor)
	inner join tbl_Tamanho on (tbl_produto.Tamanho = tbl_tamanho.IdTamanho)
	where IdCli = vIdCli;
end$$ 
...
update tbl_carrinho set Qtd = 2, TotalProd = 201.22 where IdCar = 7 and IdCli = 1;
...
select NumeroPedido, NomeCli, DataPedido, DataPrazo, TotalPedido from vwPedido;
